 ...
 * position(3) + normal(3) + uv(2), with an index buffer — ready for
 * {@code new Mesh(vertices, new int[]{3, 3, 2}, indices)}.
 *
...
public final class OBJLoader {
    ...
    public static Mesh load(String resourcePath) {
        ...
        List<Float> vertexData = new ArrayList<>();   // interleaved pos+normal+uv
        ...
        try (InputStream in = OBJLoader.class.getClassLoader().getResourceAsStream(resourcePath)) {
            if (in == null) {
                throw new RuntimeException("Model resource not found on classpath: " + resourcePath);
            }
            ...
            throw new RuntimeException("Failed to read model: " + resourcePath, e);
        }
        ...
        return new Mesh(vertices, new int[]{3, 3, 2}, indexArray);
    }
